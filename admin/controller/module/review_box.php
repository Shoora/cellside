...
class ControllerModuleReviewBox extends Controller {
	...
	public function index() {   
		$this->load->language('module/review_box');
		...
		$this->data['entry_all_review'] = $this->language->get('entry_all_review');
		...
		if (isset($this->error['header'])) {
			$this->data['error_header'] = $this->error['header'];
		} else {
			$this->data['error_header'] = array();
		}
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('module/review_box', 'token=' . $this->session->data['token'], 'SSL'),
      		'separator' => ' :: '
   		);

		$this->data['action'] = $this->url->link('module/review_box', 'token=' . $this->session->data['token'], 'SSL');
		...
		if (isset($this->request->post['review_box_module'])) {
			$this->data['modules'] = $this->request->post['review_box_module'];
		} elseif ($this->config->get('review_box_module')) { 
			$this->data['modules'] = $this->config->get('review_box_module');
		}
	...
	private function validate() {
		...
		if (isset($this->request->post['review_box_module'])) {
			...
			foreach ($this->request->post['review_box_module'] as $key => $value) {
				foreach ($value['header'] as $language_id => $title) {
