<?php
class ModelPcpbPcpb extends Model { 
	public function getDataByToken($token)
	{
		$query = $this->db->query('SELECT * FROM ' . DB_PREFIX . 'pcpb WHERE token="' . $token . '"');
		return $query->row;
	}
	...
	public function getProductPcpbOptions($product_id){
	...
	public function setSetting($key, $value, $group = 'pcpb', $store_id = 0) {
		$this->db->query("DELETE FROM " . DB_PREFIX . "setting WHERE store_id = '" . (int)$store_id . "' AND `group` = '" . $this->db->escape($group) . "' AND `key` = '" . $key . "'");
