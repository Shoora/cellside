	...
	<div class="container">
		<div id="group-content">
			...
			<div id="content"><?php echo $content_top; ?>
				...
				<?php } ?> 
				<?php if ($categories) { ?>
				<h4 class="refine"><?php echo $text_refine; ?></h4>
				<div class="category-list clearfix">
					<?php if (count($categories) <= 5) { ?>
					<ul class="col-xs-12">
						<?php foreach ($categories as $category) { ?>
						<li class="clearfix"><a href="<?php echo $category['href']; ?>"><?php echo $category['name']; ?></a></li>
						<?php } ?>
					</ul>
					<?php } else { ?>
					<?php for ($i = 0; $i < count($categories);) { ?>
					<ul class="col-xs-6 col-sm-4 col-md-3 col-lg-3">
						<?php $j = $i + ceil(count($categories) / 4); ?>
						<?php for (; $i < $j; $i++) { ?>
						<?php if (isset($categories[$i])) { ?>
						<li class="clearfix"><a href="<?php echo $categories[$i]['href']; ?>"><?php echo $categories[$i]['name']; ?></a></li>
						<?php } ?>
						<?php } ?>
					</ul>
					<?php } ?>
				...
				<div class="col-md-2">
					<div class="limit"><!--<span><?php echo $text_limit; ?></span>-->
							<select class="form-control sel" onchange="location = this.value;">
								<?php foreach ($limits as $limits) { ?>
									<?php if ($limits['value'] == $limit) { ?>
										<option value="<?php echo $limits['href']; ?>" selected="selected"><?php echo $limits['text']; ?> per page</option>
									<?php } else { ?>
										<option value="<?php echo $limits['href']; ?>"><?php echo $limits['text']; ?> per page</option>
									<?php } ?>
								<?php } ?>
							</select>
						</div>
