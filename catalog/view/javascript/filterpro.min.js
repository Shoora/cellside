...
$("#findphone").live("change", (function () {
   $('input[name="category_id"]').val($(this).val());
}));
$("#filterpro_search").live("click", (function () {
    iF();
}));

$(function () {
    var min = 0; var max = 500;
    ...
    ;
     ...
     function changePrice(val1, val2) {
         $('#min_tooltip').find('span').text('$'+val1);
         $('#max_tooltip').find('span').text('$'+val2);
     }
     function moveLabel() {
        var min = $('#slider-range a')[0];
        var min_val = $(min).css('left').split('px')[0] + 8 + 'px';
        $('#min_tooltip').css('left', min_val);
        var max_val =$('#slider-range a:last-child').css('left').split('px')[0] - 30 + 'px';
        $('#max_tooltip').css('left', max_val);
     }
...
function success(g, b) {
    ...
    $('#countProducts').html(typeof(g['totals_data'].options[0]) == "undefined" ? 0 :  g['totals_data'].options[0].t);
