...
class ControllerCatalogProduct extends Controller {
			...
			public function getTopProducts(){
				...
				switch ($range) {
					...
					case 'week':
					$sql = "SELECT SUM(op.quantity) AS quantity, op.name AS product_name, op.product_id AS product_id FROM `" . DB_PREFIX . "order_product` AS op JOIN `" . DB_PREFIX . "order` AS o ON (op.order_id = o.order_id) WHERE o.store_id = '" . $this->config->get("config_store_id") . "' AND date_added BETWEEN '" . $first_day_of_week . "' AND '" . $today_date . "' GROUP BY op.name ORDER BY SUM(op.quantity) DESC LIMIT 5";
					...
					default:
					case 'month':
					$sql = "SELECT SUM(op.quantity) AS quantity, op.name AS product_name, op.product_id AS product_id FROM `" . DB_PREFIX . "order_product` AS op JOIN `" . DB_PREFIX . "order` AS o ON (op.order_id = o.order_id) WHERE o.store_id = '" . $this->config->get("config_store_id") . "' AND date_added BETWEEN '" . $first_day_of_month . "' AND '" . $today_date . "' GROUP BY op.name ORDER BY SUM(op.quantity) DESC LIMIT 5";
					...
					case 'year':
					$sql = "SELECT SUM(op.quantity) AS quantity, op.name AS product_name, op.product_id AS product_id FROM `" . DB_PREFIX . "order_product` AS op JOIN `" . DB_PREFIX . "order` AS o ON (op.order_id = o.order_id) WHERE o.store_id = '" . $this->config->get("config_store_id") . "' AND date_added BETWEEN '" . $first_day_of_year . "' AND '" . $today_date . "' GROUP BY op.name ORDER BY SUM(op.quantity) DESC LIMIT 5";
	...
	private $error = array(); 
  	...
  	public function insert() {
    	...
    	if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->redirect($this->url->link('catalog/product_manager', 'token=' . $this->session->data['token'] . $url, 'SSL'));
    	}
  	...
  	protected function getList() {				
		...
		if (isset($this->request->get['page'])) {
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('catalog/product_manager', 'token=' . $this->session->data['token'] . $url, 'SSL'),       		
      		'separator' => ' :: '
   		);
		...
		$this->data['sort_name'] = $this->url->link('catalog/product_manager', 'token=' . $this->session->data['token'] . '&sort=pd.name' . $url, 'SSL');
		$this->data['sort_model'] = $this->url->link('catalog/product_manager', 'token=' . $this->session->data['token'] . '&sort=p.model' . $url, 'SSL');
		$this->data['sort_price'] = $this->url->link('catalog/product_manager', 'token=' . $this->session->data['token'] . '&sort=p.price' . $url, 'SSL');
		$this->data['sort_quantity'] = $this->url->link('catalog/product_manager', 'token=' . $this->session->data['token'] . '&sort=p.quantity' . $url, 'SSL');
		$this->data['sort_status'] = $this->url->link('catalog/product_manager', 'token=' . $this->session->data['token'] . '&sort=p.status' . $url, 'SSL');
		$this->data['sort_order'] = $this->url->link('catalog/product_manager', 'token=' . $this->session->data['token'] . '&sort=p.sort_order' . $url, 'SSL');
		...
		$pagination->url = $this->url->link('catalog/product_manager', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
...
if (isset($this->error['custom_title'])) {
		...
		$this->data['cancel'] = $this->url->link('catalog/product_manager', 'token=' . $this->session->data['token'] . $url, 'SSL');
